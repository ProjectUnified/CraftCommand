...
public class ArrayExecutionSource implements ExecutionSource {
    ...
    public void generateExecutionSetup(MethodSpec.Builder methodSpec, CommandModel classModel, MethodModel method, CommandModel rootModel) {
        ...
        for (ParameterModel p : method.getParameters()) {
            if (!p.isOptional()) {
                ...
                if (localRes != null) {
                    staticRequiredCount += processor.getLocalResolverMinWidth(localRes, method);
                } else {
        ...
        if (staticRequiredCount > 0) {
            methodSpec.beginControlFlow("if ($L.length < $L)", argsVar, staticRequiredCount)
                    .addStatement("throw new $T(manager.formatMessage($S, $S, $S))",
                            ...
                            "usage",
                            "Usage: %s",
                            BaseCommandProcessor.getUsage(method, classModel))
                    .endControlFlow();
        }
    ...
    public void generateParameterResolution(MethodSpec.Builder methodSpec, CommandModel classModel, MethodModel method, CommandModel rootModel, ParameterModel p, String varName, String senderVarName, int paramIndex) {
        ...
        if (resolveAnn != null && classModel.getResolverMethod(resolveAnn.value()) != null) {
            generateResolverResolution(methodSpec, classModel, method, rootModel, classModel.getResolverMethod(resolveAnn.value()), varName, senderVarName, p);
            return;
        }
    ...
    private void generateResolverResolution(MethodSpec.Builder methodSpec, CommandModel classModel, MethodModel method, CommandModel rootModel, MethodModel resolverModel, String varName, String senderVarName, ParameterModel parentParam) {
        processor.generateResolverResolution(this, methodSpec, classModel, method, rootModel, resolverModel, varName, senderVarName, parentParam, method.getSenderParameter().getName());
    }
}
