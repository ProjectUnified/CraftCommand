 ...
 */
public abstract class AbstractCommandProcessor extends AbstractProcessor {
     ...
     */
    protected static String getSimpleName(TypeName typeName) {
     ...
     */
    protected static String getDefaultPrimitiveValue(TypeMirror type) {
            ...
            if (element instanceof TypeElement) {
                ...
                if (commandModel != null) {
                    try {
                        ...
                        generateWrapper(commandModel, typeElement);
                    } catch (IOException e) {
     ...
     */
    protected void generateWrapper(CommandModel model, TypeElement typeElement) throws IOException {
        ...
        // Fields
        typeSpec.addField(model.getClassName(), "instance", Modifier.PRIVATE, Modifier.FINAL);
        typeSpec.addField(genericCommandManager, "manager", Modifier.PRIVATE, Modifier.FINAL);
        ...
        typeSpec.addMethod(MethodSpec.methodBuilder("filterSuggestions")
                .addJavadoc("Generated by {@code AbstractCommandProcessor.generateWrapper}\n")
        ...
        for (TypeName type : getDynamicResolverTypes(model)) {
            String methodName = getResolverHelperMethodName(type);
            ...
            mb.addStatement("$T<Object, $T> resolver = ($T<Object, $T>) manager.getResolver($T.class)", resolverClassName, type, resolverClassName, type, type)
                    ...
                    .endControlFlow()
                    .endControlFlow()
                    .addStatement("throw new $T(paramName, manager.formatMessage($S, $S, $S))",
        ...
        generateParamSuggestionsHelpers(typeSpec, model, model);
    ...
    protected void generateExecuteMethodBody(MethodSpec.Builder executeSpec, CommandModel model, String returnStatement) {
        ...
        generateExecutionRouting(executeSpec, model, "args", "instance", model, returnStatement);
    ...
    protected String getInstanceVarExpression(CommandModel classModel, CommandModel rootModel) {
    ...
    protected String getParamSuggestionHelperName(CommandModel classModel, MethodModel method, int index) {
    ...
    protected void generateSubcommandClassExecutors(TypeSpec.Builder typeSpec, CommandModel model, CommandModel rootModel) {
        for (CommandModel child : model.getNestedSubcommands()) {
            ...
            MethodSpec.Builder methodSpec = MethodSpec.methodBuilder(helperMethodName)
                    .addJavadoc("Generated by {@code AbstractCommandProcessor.generateSubcommandClassExecutors}\n")
                    .addModifiers(Modifier.PRIVATE)
                    .addException(Exception.class)
            ...
            generateExecutionRouting(methodSpec, child, "args", childInstanceVar, rootModel, "return");
            ...
            generateSuggestionRouting(suggestMethodSpec, child, "args", childInstanceVar, rootModel);
    ...
    protected void generateExecutionRouting(MethodSpec.Builder methodSpec, CommandModel model, String argsVar, String instanceVar, CommandModel rootModel, String returnStatement) {
        if (!model.getSubcommands().isEmpty() || !model.getNestedSubcommands().isEmpty()) {
            ...
            for (MethodModel sub : model.getSubcommands()) {
                ...
                generateMethodCall(methodSpec, model, sub, "subArgs", instanceVar, rootModel);
                methodSpec.addStatement("$L", returnStatement);
                methodSpec.endControlFlow();
            }
            methodSpec.endControlFlow();
        ...
        if (model.getDefaultMethod() != null) {
            ...
            generateMethodCall(methodSpec, model, model.getDefaultMethod(), argsVar, instanceVar, rootModel);
        } else {
        ...
        if (typeName.toString().equals("java.lang.String")) {
            ...
            if (name.equals("int") || name.equals("java.lang.Integer")) {
                ...
                methodSpec.beginControlFlow("if ($L == null)", argStrVar)
                        ...
                        .nextControlFlow("else");
                generateInlineResolve(methodSpec, typeName, varName, argStrVar);
                methodSpec.endControlFlow();
            }
        } else {
        ...
        if (isBuiltInType(typeName)) {
            if (defaultValue == null) {
                ...
                if (name.equals("java.lang.String")) {
                    methodSpec.addStatement("$L = $S", varName, defaultValue);
                } else if (name.equals("int") || name.equals("java.lang.Integer")) {
                    try {
                        methodSpec.addStatement("$L = $L", varName, Integer.parseInt(defaultValue));
                    } catch (NumberFormatException e) {
                        methodSpec.addStatement("$L = $T.parseInt($S)", varName, Integer.class, defaultValue);
                    }
                } else if (name.equals("double") || name.equals("java.lang.Double")) {
                    try {
                        methodSpec.addStatement("$L = $L", varName, Double.parseDouble(defaultValue));
                    } catch (NumberFormatException e) {
                        methodSpec.addStatement("$L = $T.parseDouble($S)", varName, Double.class, defaultValue);
                    }
                } else if (name.equals("float") || name.equals("java.lang.Float")) {
                    try {
                        methodSpec.addStatement("$L = $Lf", varName, Float.parseFloat(defaultValue));
                    } catch (NumberFormatException e) {
                        methodSpec.addStatement("$L = $T.parseFloat($S)", varName, Float.class, defaultValue);
                    }
                } else if (name.equals("long") || name.equals("java.lang.Long")) {
                    try {
                        methodSpec.addStatement("$L = $LL", varName, Long.parseLong(defaultValue));
                    } catch (NumberFormatException e) {
                        methodSpec.addStatement("$L = $T.parseLong($S)", varName, Long.class, defaultValue);
                    }
                } else if (name.equals("short") || name.equals("java.lang.Short")) {
                    try {
                    ...
                    methodSpec.addStatement("$L = $L", varName, Boolean.parseBoolean(defaultValue));
                    ...
                    generateInlineResolve(methodSpec, typeName, varName, "tempDefault");
                }
            }
        } else {
            if (typeName.isPrimitive()) {
    ...
    protected void generateMethodCall(MethodSpec.Builder methodSpec, CommandModel classModel, MethodModel method, String argsVar, String instanceVar, CommandModel rootModel) {
        ...
        if (senderResolver != null) {
            ...
            if (!isSenderBaseType(senderParamTypeName)) {
                methodSpec.addComment("Verify that command sender is an instance of " + senderParamTypeName.toString());
                methodSpec.beginControlFlow("if (!(sender instanceof $T))", senderParamTypeName)
                        .addStatement("throw new $T($T.class, manager.formatMessage($S, $S, $S))",
                                InvalidSenderException.class,
                                senderParamTypeName,
                                "invalid-sender",
                                "Only %s can execute this command.",
                                getSimpleName(senderParamTypeName))
                        .endControlFlow();
                methodSpec.addStatement("$T $L = ($T) sender", senderParamTypeName, senderVarName, senderParamTypeName);
            } else {
        ...
        // 2. Validate Minimum Argument Count Statically
        int staticRequiredCount = 0;
        for (ParameterModel p : method.getParameters()) {
            if (!p.isOptional()) {
                ExecutableElement localRes = findLocalResolver(classModel, p, rootModel);
                if (localRes != null) {
                    staticRequiredCount += getLocalResolverMinWidth(localRes);
                } else {
            ...
            if (localResolver != null) {
                ...
                if (!p.isOptional()) {
                    methodSpec.beginControlFlow("if ($L + $L > $L.length)", argIdxVar, minWidth_i, argsVar)
                            .addStatement("throw new $T($S, manager.formatMessage($S, $S, $S))",
                                    MissingArgumentException.class,
                                    p.getName(),
                                    "missing-argument",
                                    "Missing arguments for parameter: %s",
                ...
                for (int j = resolverStartIndex; j < resolverParams.size(); j++) {
                    ...
                    Optional optionalAnn =
                            rp.getAnnotation(Optional.class);
                ...
                if (isBuiltInType(pTypeName)) {
                        ...
                        if (width == 1) {
                            if (pTypeName.toString().equals("java.lang.String")) {
                                ...
                                methodSpec.beginControlFlow("if (argStr_$L == null)", i)
                                        .addStatement("$L = $L", varName, defVal)
                                        .nextControlFlow("else");
                                generateInlineResolve(methodSpec, pTypeName, varName, "argStr_" + i);
                                methodSpec.endControlFlow();
                            }
                        } else {
                            methodSpec.addStatement("$T $L", pTypeName, varName);
                            generateInlineResolveMulti(methodSpec, pTypeName, varName, argsVar, argIdxVar, senderVarName, i);
                    ...
                    String resolverMethodName = getResolverHelperMethodName(pTypeName.isPrimitive() ? pTypeName.box() : pTypeName);
    ...
    protected void generateSuggestionRouting(MethodSpec.Builder methodSpec, CommandModel model, String argsVar, String instanceVar, CommandModel rootModel) {
        ...
        if (hasChildren) {
            ...
            if (model.getDefaultMethod() != null && !model.getDefaultMethod().getParameters().isEmpty()) {
                ...
                if (!isParamSuggestionEmpty(p0)) {
                    onSuggestionAdd(methodSpec, model.getDefaultMethod().getElement(), () -> {
                        String helperName = getParamSuggestionHelperName(model, model.getDefaultMethod(), 0);
            ...
            for (MethodModel sub : model.getSubcommands()) {
                ...
                generateSubcommandRoutingTabComplete(methodSpec, model, sub, "subArgs");
                methodSpec.endControlFlow();
            }
            methodSpec.endControlFlow();
        }
        ...
        if (model.getDefaultMethod() != null) {
            ...
            generateSubcommandRoutingTabComplete(methodSpec, model, model.getDefaultMethod(), argsVar);
        } else {
    ...
    protected void generateSubcommandRoutingTabComplete(MethodSpec.Builder methodSpec, CommandModel classModel, MethodModel method, String argsVar) {
        ...
        for (int i = 0; i < paramCount; i++) {
            ParameterModel p = method.getParameters().get(i);
            TypeName pTypeName = TypeName.get(p.getType());
            String helperName = getParamSuggestionHelperName(classModel, method, i);
            ...
            if (localResolver != null) {
                ...
                methodSpec.addStatement("int width_$L = $L", i, maxWidth);
            } else {
                if (isBuiltInType(pTypeName)) {
                    methodSpec.addStatement("int width_$L = 1", i);
                } else {
                    methodSpec.addStatement("int width_$L = manager.getResolver($T.class).getWidth()", i, pTypeName.isPrimitive() ? pTypeName.box() : pTypeName);
                }
            }

            methodSpec.beginControlFlow("if (index >= tempIdx && index < tempIdx + width_$L)", i);
            ...
            methodSpec.endControlFlow();
            methodSpec.addStatement("tempIdx += width_$L", i);
        }
        methodSpec.addStatement("return $T.emptyList()", Collections.class);
    }
    ...
    protected void generateParamSuggestionsHelpers(TypeSpec.Builder typeSpec, CommandModel model, CommandModel rootModel) {
        if (model.getDefaultMethod() != null) {
            for (int i = 0; i < model.getDefaultMethod().getParameters().size(); i++) {
                ParameterModel p = model.getDefaultMethod().getParameters().get(i);
                if (!isParamSuggestionEmpty(p)) {
                    typeSpec.addMethod(generateParamSuggestionsHelper(model, model.getDefaultMethod(), p, i, rootModel));
                }
            }
        }
        for (MethodModel sub : model.getSubcommands()) {
            for (int i = 0; i < sub.getParameters().size(); i++) {
                ...
                if (!isParamSuggestionEmpty(p)) {
                    typeSpec.addMethod(generateParamSuggestionsHelper(model, sub, p, i, rootModel));
                }
            }
        }
        for (CommandModel child : model.getNestedSubcommands()) {
            generateParamSuggestionsHelpers(typeSpec, child, rootModel);
        }
    }

    protected MethodSpec generateParamSuggestionsHelper(CommandModel classModel, MethodModel method, ParameterModel p, int index, CommandModel rootModel) {
        String helperName = getParamSuggestionHelperName(classModel, method, index);
        MethodSpec.Builder methodSpec = MethodSpec.methodBuilder(helperName)
                .addJavadoc("Generated by {@code AbstractCommandProcessor.generateParamSuggestionsHelper}\n")
                .addModifiers(Modifier.PRIVATE)
                .returns(ParameterizedTypeName.get(List.class, String.class))
                .addParameter(Object.class, "sender")
                .addParameter(String[].class, "args")
        ...
        if (provider != null) {
            ...
            TypeName pTypeName = TypeName.get(p.getType());
            if (pTypeName.toString().equals("boolean") || pTypeName.toString().equals("java.lang.Boolean")) {
                methodSpec.addStatement("$T list = new $T()", ParameterizedTypeName.get(List.class, String.class), ArrayList.class);
                methodSpec.addStatement("String lower = current.toLowerCase()");
                methodSpec.addStatement("if (\"true\".startsWith(lower)) list.add(\"true\")");
                methodSpec.addStatement("if (\"false\".startsWith(lower)) list.add(\"false\")");
    ...
    protected ExecutableElement findLocalResolver(CommandModel classModel, ParameterModel p, CommandModel rootModel) {
    ...
    protected CommandModel findModelForClass(CommandModel current, TypeElement targetClass) {
    ...
    private void generateInlineResolve(MethodSpec.Builder methodSpec, TypeName typeName, String varName, String argStrVar) {
        String name = typeName.toString();
        if (name.equals("java.lang.String")) {
            methodSpec.addStatement("$L = $L", varName, argStrVar);
        } else if (name.equals("int") || name.equals("java.lang.Integer")) {
            ...
            generatePlatformInlineResolve(methodSpec, typeName, varName, argStrVar);
        }
    }

    private void generateInlineResolveMulti(MethodSpec.Builder methodSpec, TypeName typeName, String varName, String argsVar, String argIdxVar, String senderVar, int i) {
        if (isPlatformBuiltInType(typeName)) {
            generatePlatformInlineResolveMulti(methodSpec, typeName, varName, argsVar, argIdxVar, senderVar, i);
        }
    }
     ...
     */
    private String getResolverHelperMethodName(TypeName typeName) {
